...
def feature_select():
    ...
    """
    Function to determine the most important features to use in ML classification from the dataset.
    """

    # Establish command-line interface
    parser = argparse.ArgumentParser(description='RFECV feature selection.')  # Initialise parser
    ...
    rfe_cv = StratifiedKFold(n_splits=args.K_folds, shuffle=True, random_state=args.random_state)

    # Tomek Links undersampling
    tl = TomekLinks(sampling_strategy='majority', n_jobs=args.threads)
    ...
    feat_indexes = [i for i, b in enumerate(pipeline.named_steps['rfe'].support_) if b]

    # Save feat_indexes to file
    with open('/data/home/bt211037/dissertation/selected_feature_indexes.txt', 'w') as f:
        for item in feat_indexes:
            f.write(f'{item} \n')
